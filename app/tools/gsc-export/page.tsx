...
export default function GscExportPage() {
    ...
    // Separate sort config for child rows (pages) in the pivot table
...
const analyzeCannibalization = (rows: GscRow[]) => {
    ...
    rows.forEach(row => {
        ...
        if (!queryMap[query]) {
            // Ensure each query only stores unique URLs
            queryMap[query] = {
                query,
                pageCount: 0,
                totalClicks: 0,
        ...
        if (existingPage) {
            // If duplicate, aggregate clicks/impressions
            existingPage.clicks += row.clicks;
            existingPage.impressions += row.impressions;
            existingPage.position = (existingPage.position + row.position) / 2; // average position
            existingPage.ctr = (existingPage.ctr + row.ctr) / 2; // average ctr
        } else {
        ...
        // Update aggregates regardless of duplicate handling
                                                            ...
                                                            <tr className="bg-gray-50 dark:bg-gray-900/50">
                                                                <td colSpan={5} className="px-6 py-4">
                                                                    <div className="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                                                                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700" style={{ width: '100%' }}>
